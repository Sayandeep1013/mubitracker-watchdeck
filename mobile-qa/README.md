...
maestro test mobile-qa/flows/filters-and-presets.yaml
...
|---|---|---|---|
| `auth-guard-offline.yaml` | Offline cold launch reaches the login screen instead of hanging on a blank view | 0.1 | run and passing — re-verified `2026-08-14` after fixing 3 real bugs in `subflows/open-project.yaml`/`sign-in.yaml` (see git log) |
...
| `undo-after-review-later.yaml` | The undo pill names the title that was just sent to Review Later, and undo restores that title | 0.5 | run and passing pre-3.6; swipe direction updated for 3.6; Undo-chip text assertions updated `2026-08-14` for the new corner-chip UI (visible text is now just "Undo", title moved to the accessibilityLabel) — **not re-run as a file since either edit** |
| `deck-gesture-map-and-buttons.yaml` | All four swipe directions (←/→/↑/↓) match the canonical web gesture map | 3.2, 3.5, 3.6 | Rewritten `2026-08-14` — the fallback-button section is **gone** (buttons were removed from the app entirely; deck is now solely gesture-driven, a deliberate product decision). The 4-direction swipe check itself was verified working via ad hoc Maestro commands this session (not by running this exact file) — **still not run as a file** |
| `toast-and-keyboard.yaml` | Offline classify shows the "Saved offline" toast (not a silent failure); the review modal's Save button stays reachable above the keyboard | 3.3, 3.4 | Nav path fixed `2026-08-14` (Review Later moved from the bottom tab bar into the hamburger drawer — flow now opens the menu first) — **still not run as a file** |
| `friends-ui.yaml` | Friends tab reachable, first-run discoverability nudge + Copy handle, all four segmented tabs render their empty states, add-friend modal opens and searches, notifications modal opens empty | 4.2 | Nav path fixed `2026-08-14` (Friends moved into the hamburger drawer — flow now opens the menu first) — the equivalent navigation was verified working via ad hoc Maestro commands this session, but **this exact file has still never been run**. Single-device scope only, see file header for what it can't cover |
| `filters-and-presets.yaml` | Filters modal opens from the Deck header, Type/Genre chips select, a named preset saves and persists, Apply Filters closes the modal and the header badge reflects the active count, the preset survives a second open | 4.3 | **written, never run** |
